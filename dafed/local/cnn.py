...
class CNN_FMNIST(nn.Module):
    def __init__(self):
        ...
        nn.Dropout2d(),
        nn.Linear(512, 10),)
        self.discriminator = Discriminator(128*4*4,512)
    def forward(self, x):
        out = self.layer1(x)
        out = self.layer2(out)
        self.hidden_x=out = out.view(-1, 128*4*4)
        out = self.fc(out)
        return out
...
class CNN_FEMNIST(nn.Module):
    ...
    def __init__(self, only_digits=False):
        ...
        self.discriminator = Discriminator(9216, 128)
    ...
    def forward(self, x):
        ...
        x = self.dropout_1(x)
        self.hidden_x = x = self.flatten(x)
        x = self.linear_1(x)
        x = self.relu(x)
        x = self.dropout_2(x)
        x = self.linear_2(x)
...
class CNN_MNIST(nn.Module):
    def __init__(self):
        ...
        self.discriminator = Discriminator(1024, 512)

    def forward(self, x):
        x = self.pool(self.conv1(x))
        x = self.pool(self.conv2(x))
        self.hidden_x = x = x.view(x.shape[0], -1)
...
class CNN_CIFAR10(nn.Module):
    ...
    def __init__(self):
        ...
        self.discriminator = Discriminator(120, 120)

    def forward(self, x):
        x = self.pool(F.relu(self.conv1(x)))
        x = self.pool(F.relu(self.conv2(x)))
        self.hidden_x = x = torch.flatten(x, 1)  # flatten all dimensions except batch
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
        x = self.fc3(x)
        return x
...
class AlexNet_CIFAR10(nn.Module):
    ...
    def __init__(self, num_classes=10):
        ...
        self.discriminator = Discriminator(256 * 2 * 2, 4096)
    ...
    def forward(self, x):
        x = self.features(x)
        self.hidden_x = x = x.view(x.size(0), 256 * 2 * 2)
        x = self.classifier(x)
        return x